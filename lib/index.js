    ...
    if (overflow.adjustX) {
      ...
      if (isFailX(elFuturePos, elRegion, visibleRect)) {
        ...
        if (newPoints[0].charAt(1) === 'c') {
          XregionReversal = _utils2['default'].merge(visibleRect, {
            left: refNodeOffset.left - elRegion.width / 2,
            right: visibleRect.right + elRegion.width / 2
          });
        } else {
          XregionReversal = _utils2['default'].merge(visibleRect, _defineProperty({}, newPoints[0].charAt(1) === 'l' ? 'left' : 'right', (0, _getAlignOffset2['default'])(refNodeRegion, newPoints[1]).left));
        }
    ...
    if (overflow.adjustY) {
      ...
      if (isFailY(elFuturePos, elRegion, visibleRect)) {
        ...
        if (_newPoints[0].charAt(0) === 'c') {
          YRegionReversal = _utils2['default'].merge(visibleRect, {
            top: refNodeOffset.top - elRegion.height / 2,
            bottom: visibleRect.bottom + elRegion.height / 2
          });
        } else {
          YRegionReversal = _utils2['default'].merge(visibleRect, _defineProperty({}, _newPoints[0].charAt(0) === 't' ? 'top' : 'bottom', (0, _getAlignOffset2['default'])(refNodeRegion, _newPoints[1]).top));
        }
